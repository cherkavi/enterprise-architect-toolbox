...
%% The Categories (Big Bones)
    People([People]) --> Spine
    Process([Process]) --> Spine
    Tools([Tools]) --> Spine
    Environment([Environment]) --> Spine

%% Causes under People
    NoTraining[Poor Training] -.-> People
    LowMotivation[Low Motivation] -.-> People
    NoSkill[Missing Key Skills] -.-> People

%% Causes under Process
    UnclearSteps[Unclear Steps] -.-> Process
    Bottlenecks[Approval Bottlenecks] -.-> Process
    BadEstimation[Poor Time Estimation] -.-> Process

%% Causes under Tools
    OldSoftware[Outdated Software] -.-> Tools
    SlowComputers[Slow Computers] -.-> Tools
    BrokenAccess[Broken Access Permissions] -.-> Tools

%% Causes under Environment
    Distractions[Constant Distractions] -.-> Environment
    BadOfficeLayout[Bad Office Layout] -.-> Environment
    UnclearGoals[Unclear Company Goals] -.-> Environment

%% Root Causes (Small Bones)
    NoHiring[No Hiring Budget] -.-> NoSkill
    NoDocs[Missing Documentation] -.-> UnclearSteps

%% Styling
    style Problem fill:#f96,stroke:#333,stroke-width:2px,color:white,font-weight:bold
    style People fill:#6cf,stroke:#333
    style Process fill:#6cf,stroke:#333
    style Tools fill:#6cf,stroke:#333
    style Environment fill:#6cf,stroke:#333
```
